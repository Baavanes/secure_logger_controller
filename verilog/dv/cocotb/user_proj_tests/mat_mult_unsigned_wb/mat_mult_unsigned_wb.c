...
//
// Matrix Multiplier Unsigned Mode Test with Final Result Writing
...
void main()
{
    ...
    *((volatile uint32_t *)MATMUL_CTRL) = CTRL_START;
    ...
    uint8_t result = *((volatile uint32_t *)MATMUL_C_BASE); // 8-bit result

    uint8_t passed = (result == 23);

    // Prepare the final result for write (upper 24 bits = 110000000000000000000000, lower 8 bits = result)
    uint32_t final_result = (0xC00000 << 8) | result;  // Keep result as 8-bit

    // Write final result to the desired Wishbone address (0x30000000)
    *((volatile uint32_t *)0x30000000) = final_result;

    // Perform another Wishbone write with 32'h40000000
    uint32_t second_write_data = 0x40000000;
    *((volatile uint32_t *)0x30000000) = second_write_data;  // Write to the same address
    
    wait_cycles(300);

    // Perform Wishbone read for address 0x30000000
    uint32_t read_data = *((volatile uint32_t *)0x30000000);

    // Verify the result of the read operation (check if read data is 0)
    if (read_data == 0) {
        // Final result matches 0, indicate success
        ManagmentGpio_write(0);
    } else {
        // Read data mismatch, indicate failure
        ManagmentGpio_write(1);
    }
